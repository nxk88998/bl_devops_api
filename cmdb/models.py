...
class Server(models.Model):
    idc = models.ForeignKey(Idc, on_delete=models.PROTECT, verbose_name="IDC机房") # 服务器与机房一对多关系
    server_group = models.ManyToManyField(ServerGroup, verbose_name="主机分组") # 服务器与分组多对多关系
    credential = models.ForeignKey(Credential, blank=True, null=True, on_delete=models.PROTECT, verbose_name="SSH凭据") # 服务与凭据一对多关系
    ...
    cpu_num = models.CharField(max_length=10, blank=True, verbose_name="CPU")
    cpu_model = models.CharField(max_length=100, blank=True, verbose_name="CPU型号")
    memory = models.CharField(max_length=30, blank=True, verbose_name="内存")
    ...
    is_verified = models.CharField(max_length=10, choices=(('verified','已验证'),('unverified','未验证')), default='unverified', verbose_name="SSH验证状态")
